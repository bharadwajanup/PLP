...
(define return-k
  (lambda(v-exp env)
    `(return ,v-exp ,env)));(lambda(v)  (value-of-cps v-exp env v))))
...
(define app-outer-k
  (lambda(rand^ env^ k^)
    `(app-outer-k ,rand^ ,env^ ,k^)));(lambda(v) (value-of-cps rand^ env^ (app-inner-k v k^)))))
...
(define mult-inner-k
  (lambda (v^ k^)
    `(mult-inner-k ,v^ ,k^)));(lambda(v)
     ; (apply-k k^ (* v^ v)))))

(define mult-outer-k
  (lambda(x2^ env^ k^)
    `(mult-outer-k ,x2^ ,env^ ,k^)));(lambda(v)  (value-of-cps x2^ env^ (mult-inner-k v k^)))))
