...
(define value-of-cps
  (lambda (expr env k)
    (match expr
      ...
      [`(capture ,body)  (value-of-cps body (lambda (y k^) (if (zero? y) (apply-k k^ k) (apply-env env (sub1 y) k^))) k)]
      ...
      [`(let ,e ,body) (value-of-cps e env (lambda(x^)
                         (value-of-cps body (lambda (y k^) (if (zero? y) (apply-k k^ x^) (apply-env env (sub1 y) k^))) k)))]
      [`(var ,expr) (apply-env env expr k)]
      [`(lambda ,body) (apply-k k (lambda (a k^) (value-of-cps body (lambda (y k^^) (if (zero? y) (k^^ a) (apply-env env (sub1 y) k^^))) k^)))]
