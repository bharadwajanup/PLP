...
(define value-of-cps
  (lambda (expr env k)
    (match expr
      [`(const ,expr) (k expr)]
      [`(mult ,x1 ,x2) (value-of-cps x1 env (lambda(x^) (k (value-of-cps x2 env (lambda(x^^) (* x^ x^^))))))]
      ...
      [`(capture ,body)  (value-of-cps body (lambda (y k^) (if (zero? y) (k^ k) (env (sub1 y) k^))) k)]
      ...
      [`(let ,e ,body) (value-of-cps e env (lambda(x^)
                         (value-of-cps body (lambda (y k^) (if (zero? y) (k^ x^) (env (sub1 y) k^))) k)))]
      [`(var ,expr) (env expr k)]
      [`(lambda ,body) (k (lambda (a k^) (value-of-cps body (lambda (y k^^) (if (zero? y) (k^^ a) (env (sub1 y) k^^))) k^)))]
      [`(app ,rator ,rand) (value-of-cps rator env (lambda(x^) (value-of-cps rand env (lambda(x^^)  (x^ x^^ k)))))])))
