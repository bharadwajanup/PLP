...
(define value-of-cps
  (lambda (exp env k)
    (union-case exp expr
      ...
      [(mult x1 x2) (value-of-cps x1 env (kt_mult-outer-k x2 env k))]
      [(sub1 x) (value-of-cps x env (kt_sub1-k k))]
      [(zero x) (value-of-cps x env (kt_zero-k k))]
      [(if test conseq alt) (value-of-cps test env (kt_if-k conseq alt env k))]
      [(capture body)  (value-of-cps body (envr_extend-env env k) k)]
      [(return k-exp v-exp) (value-of-cps k-exp env (kt_return-k v-exp env))]
...
(define apply-k
  (lambda(k v)
    (union-case k kt
      ...
      [(mult-outer-k x2^ env^ k^) (value-of-cps x2^ env^ (kt_mult-inner-k v k^))]
